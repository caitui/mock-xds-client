...
func NewAdsClient(config *config.MockXdsClientConfig) (*ADSClient, error) {
...
func (adsClient *ADSClient) Start() {
	if adsClient.config == nil {
		log.Println("[xds] no xds config parsed, no xds action")
		return
	}
	_ = adsClient.connect()
...
// TODO 不定时发起ads请求，只是连接保活
func (adsClient *ADSClient) sendRequestLoop() {
	log.Println("[xds] [ads client] send request, start with cds")
	// start a directly-timer
	t := time.NewTimer(0)
	for {
		select {
		case <-adsClient.stopChan:
			log.Println("[xds] [ads client] send request loop shutdown")
			adsClient.stopStreamClient()
			return
		case <-t.C:
			c := adsClient.GetStreamClient()
			if c == nil {
				log.Println("[xds] [ads client] stream client closed, sleep 1s and wait for reconnect")
				time.Sleep(time.Second)
				adsClient.reconnect()
			} else if err := c.Send(adsClient.config.InitAdsRequest()); err != nil {
				log.Printf("[xds] [ads client] send thread request cds fail!auto retry next period. err: %s\n", err.Error())
				adsClient.reconnect()
			}
...
func (adsClient *ADSClient) receiveResponseLoop() {
	for {
		select {
		case <-adsClient.stopChan:
			log.Println("[xds] [ads client] receive response loop shutdown")
			return
		default:
			c := adsClient.GetStreamClient()
			if c == nil {
				log.Println("[xds] [ads client] try receive response: stream client closed")
				time.Sleep(time.Second)
				continue
			}
			resp, err := c.Recv()
			if err != nil {
				log.Printf("[xds] [ads client] get resp error: %s\n", err.Error())
				time.Sleep(time.Second)
				continue
			}
...
func (adsClient *ADSClient) reconnect() {
	adsClient.stopStreamClient()
	log.Println("[xds] [ads client] close stream client before retry")
	...
	for {
		if !disableReconnect {
			...
			if err == nil {
				log.Println("[xds] [ads client] stream client reconnected")
				return
			}
			log.Printf("[xds] [ads client] stream client reconnect failed %s,  retry after %v\n", err.Error(), interval)
		}
		// sleep random
